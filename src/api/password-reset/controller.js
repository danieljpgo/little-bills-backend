...
export const create = ({ bodymen: { body: { email } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const link = `${linkPassword.replace(/\/$/, '')}/${token}`
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your backend-littlebills account.<br>
        ...
        &mdash; backend-littlebills Team
      `
      return sendMail({ toEmail: email, subject: 'backend-littlebills - Password Reset', content })
    })
